...
subroutine ice_cat_transport(CAS, TrReg, dt_slow, nsteps, G, US, IG, CS, uc, vc, mca_tot, uh_tot, vh_tot)
  ...
  if (CAS%dt_sum <= 0.0) then
    call set_massless_SIS_tracers(CAS%m_snow, TrReg, G, IG, do_ice=.false.)
    call set_massless_SIS_tracers(CAS%m_ice, TrReg, G, IG, do_snow=.false.)
...
!    rdg_vosh    ! rate of ice mass shifted from level to ridged ice [R Z T-1 ~> kg m-2 s-1]
  ...
  enddo ; enddo ; enddo
  call set_massless_SIS_tracers(mca0_snow, TrReg, G, IG, do_ice=.false.)
  call set_massless_SIS_tracers(mca0_ice, TrReg, G, IG, do_snow=.false.)
